...
from pro_db import models
from pro_db.models import a as modelA
from pro_db.models import b as modelB
from pro_db.models import c as modelC
from pro_db.models import info as modelInfo
from pro_db.models import staff as modelStaff
from pro_db.models import soft as modelSoft
...
def printWord(request):
    ...
    # 方式1
    modelWhich='pro_a'
    Sql =str('select * from '+modelWhich)#  Sql = "select * from pro_a"
...
def get_pro_json(es,request):
    data = {}
    for e in es:
        if (str(request.path)=='/get_pro_a' or str(request.path)=='/get_pro_b' or str(request.path)=='/get_pro_c'):
            print (1)
            # 判断是否有
            if (e.imgs):
                iFimgs = e.imgs.url
            ...
            print (2)
            if (e.portrait):
                iFPortrait = e.portrait.url
            else:
                iFPortrait = ''
            data_each = {"id": e.id, "name": e.name, "portrait": iFPortrait, "QQ": e.QQ,"blog": e.blog,"job": e.job,"adept": e.adept,"other": e.other}
        ...
        elif(str(request.path)=='/get_pro_info'):
            print (4)
            data_each = {"id": e.id, "name": e.name, "pwd": e.pwd, "save": e.save}
...
def get_pro_content(request):
    tableN = [str(request.path)[5:]]
    modelN = isWhichTable(tableN)
    es = modelN.objects.all()
